...
use super::cursor_diagnostics::{
    log_coordinate_conversion, log_line_height_calculation, log_mouse_click, log_position_flow,
};
...
impl MarkdownEditor {
    ...
    pub(super) fn handle_mouse_down(
        ...
        if event.modifiers.shift {
            ...
            log_position_flow(
                "2-BEFORE_HANDLE_CLICK",
                character_position,
                "passing to handle_click_with_count",
            );
    ...
    fn convert_screen_to_text_point(
        ...
        eprintln!(
            "  📍 Raw screen coordinates: ({:.1}, {:.1})px",
            screen_point.x.0, screen_point.y.0
        );
        eprintln!(
            "  📏 Our calculated bounds: top={:.1}px, left={:.1}px",
        ...
        if relative_y.0 < 0.0 {
            eprintln!(
                "  ⚠️  PROBLEM: Negative Y coordinate! Click is ABOVE the calculated text area"
            );
            eprintln!(
                "     This means our top offset ({:.1}px) is too large",
                content_bounds.top_offset.0
            );
        }

        if relative_x.0 < 0.0 {
            eprintln!(
                "  ⚠️  PROBLEM: Negative X coordinate! Click is LEFT of the calculated text area"
            );
            eprintln!(
                "     This means our left offset ({:.1}px) is too large",
                content_bounds.left_offset.0
            );
        }
        ...
        eprintln!(
            "  📊 Screen position input: ({:.1}, {:.1})px",
            screen_pos.x, screen_pos.y
        );
        eprintln!(
            "  📝 Calculated text point: row={}, col={}",
            text_point.row, text_point.column
        );
        eprintln!("  🔍 Quick validation:");
        eprintln!(
            "     - Y {:.1}px should roughly map to line {}",
            relative_y.0,
            (relative_y.0 / 24.0).floor() as u32
        );
        eprintln!("     - Actual line calculated: {}", text_point.row);

        if text_point.row != (relative_y.0 / 24.0).floor() as u32 {
            eprintln!(
                "  ⚠️  LINE MISMATCH! Expected ~{}, got {}",
                (relative_y.0 / 24.0).floor() as u32,
                text_point.row
            );
        }
    ...
    fn convert_screen_position_to_text_point(
        &self,
        ...
        for (idx, line_content) in lines.iter().enumerate() {
            ...
            eprintln!(
                "  Line {}: Y range {:.1}px to {:.1}px (height: {:.1}px)",
                idx,
                current_y_offset,
            ...
            if screen_pos.y >= current_y_offset && screen_pos.y < current_y_offset + line_height {
                eprintln!(
                    "  ✅ MATCH: Click at {:.1}px falls in line {} range",
                    screen_pos.y, idx
                );
                row = idx as u32;
                break;
    ...
    fn calculate_line_height_for_content_pixels(&self, line_content: &str) -> f32 {
        ...
        if line_content.starts_with('#') {
            ...
            if heading_level <= 6 {
                ...
                let heading_font_size = self
                    .hybrid_renderer
                    .get_scalable_font_size_for_heading_level(heading_level, base_font_size);
                let heading_line_height = self
                    .hybrid_renderer
                    .get_line_height_for_font_size(heading_font_size);
    ...
    fn calculate_column_from_x_position(
        &self,
        line_content: &str,
        x_position: f32,
        _row: u32,
        ...
        for col in 0..=line_chars.len() {
            ...
            eprintln!(
                "  🔍 Column {}: text='{}' width={:.1}px distance={:.1}px",
                col,
                text_up_to_col.replace('\n', "⏎"),
                measured_width,
                distance
    ...
    fn calculate_column_from_x_position_with_gpui(
        ...
        let display_content = self.hybrid_renderer.get_display_content(
            line_content,
            cursor_pos_in_line,
            line_selection.clone(),
        );
        let text_runs = self.hybrid_renderer.generate_mixed_text_runs(
            line_content,
            cursor_pos_in_line,
            line_selection,
        );
    ...
    fn measure_text_width(&self, text: &str, line_content: &str) -> f32 {
        ...
        eprintln!(
            "DEBUG MEASURE: Improved measurement for '{}' with font_size={}",
            text, font_size
        );
    ...
    fn measure_text_width_improved_approximation(&self, text: &str, font_size: f32) -> f32 {
        ...
        eprintln!(
            "DEBUG MEASURE: Refined approximation result: {:.1}px for '{}' ({} chars)",
            total_width,
            text,
            text.chars().count()
        );
    ...
    fn calculate_character_offset_from_x_position(
        &self,
        line_content: &str,
        x_position: Pixels,
    ...
    fn estimate_x_position_for_character_offset(
        &self,
        line_content: &str,
        char_offset: usize,
        line_start_pos: usize,
        ...
        match click_count {
            1 => {
                ...
                log_position_flow(
                    "4-BEFORE_MOUSE_DOWN",
                    position,
                    "passing to handle_mouse_down_at_position",
                );
    ...
    pub fn handle_mouse_down_at_position(&mut self, position: usize) -> bool {
        ...
        log_position_flow(
            "6-AFTER_CLAMPING",
            clamped_position,
            &format!("clamped to max_pos={}", max_pos),
        );
        ...
        log_position_flow(
            "7-BEFORE_SET_CURSOR",
            old_cursor_pos,
            "current cursor position before change",
        );
        ...
        log_position_flow(
            "8-AFTER_SET_CURSOR",
            new_cursor_pos,
            "cursor position after set_cursor_position",
        );
        ...
        if new_cursor_pos != clamped_position {
            log_position_flow(
                ...
                &format!(
                    ...
                    clamped_position,
                    new_cursor_pos,
                    new_cursor_pos as i32 - clamped_position as i32
                ),
            );
        }
    ...
    #[cfg(test)]
    pub fn test_calculate_column_from_x_position(
        &self,
        line_content: &str,
        x_position: f32,
    ) -> u32 {
...
pub mod tests {
    ...
    pub fn create_test_editor() -> MarkdownEditor {
        ...
        unimplemented!(
            "Use specific method tests instead - see test_character_width_approximation_accuracy"
        )
    }
    ...
    fn test_text_content_bounds_calculation() {
        ...
        assert!(
            expected_top.0 < 200.0,
            "Top offset should be reasonable (< 200px)"
        );
    }
    ...
    fn test_coordinate_bounds_values() {
        ...
        assert_eq!(
            relative_x, 84.0,
            "X coordinate transformation should be correct"
        );
        assert_eq!(
            relative_y, 76.0,
            "Y coordinate transformation should be correct"
        );

        // Test edge cases
        assert!(
            relative_x > 0.0,
            "X should be positive for clicks inside content area"
        );
        assert!(
            relative_y > 0.0,
            "Y should be positive for clicks below top offset"
        );
    }
}
