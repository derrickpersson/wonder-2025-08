#[derive(Debug, Clone, PartialEq)]
pub enum InputEvent {
    Character(char),
    Backspace,
    Delete,
    ArrowLeft,
    ArrowRight,
    ArrowUp,
    ArrowDown,
    ShiftArrowLeft,
    ShiftArrowRight,
    CmdArrowLeft,
    CmdArrowRight,
    CmdShiftArrowLeft,
    CmdShiftArrowRight,
    CmdArrowUp,
    CmdArrowDown,
    CmdShiftArrowUp,
    CmdShiftArrowDown,
    Home,
    End,
    PageUp,
    PageDown,
    Enter,
    Tab,
    CmdA,
    CmdC,
    CmdX,
    CmdV,
    CmdShiftV,
    CtrlC,
    CtrlX,
    CtrlV,
    CtrlShiftV,
    OptionLeft,
    OptionRight,
    ShiftHome,
    ShiftEnd,
    CtrlShiftHome,
    CtrlShiftEnd,
    CmdShiftHome,
    CmdShiftEnd,
    FnUp,
    FnDown,
    ShiftPageUp,
    ShiftPageDown,
    CtrlShiftUp,
    CtrlShiftDown,
    Escape,
}

impl InputEvent {
    pub fn from_char(ch: char) -> Self {
        InputEvent::Character(ch)
    }
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub enum SpecialKey {
    Backspace,
    Delete,
    ArrowLeft,
    ArrowRight,
    ArrowUp,
    ArrowDown,
    ShiftArrowLeft,
    ShiftArrowRight,
    CmdArrowLeft,
    CmdArrowRight,
    CmdShiftArrowLeft,
    CmdShiftArrowRight,
    CmdArrowUp,
    CmdArrowDown,
    CmdShiftArrowUp,
    CmdShiftArrowDown,
    Home,
    End,
    PageUp,
    PageDown,
    Enter,
    Tab,
    CmdA,
    CmdC,
    CmdX,
    CmdV,
    CmdShiftV,
    CtrlC,
    CtrlX,
    CtrlV,
    CtrlShiftV,
    OptionLeft,
    OptionRight,
    ShiftHome,
    ShiftEnd,
    CtrlShiftHome,
    CtrlShiftEnd,
    CmdShiftHome,
    CmdShiftEnd,
    FnUp,
    FnDown,
    ShiftPageUp,
    ShiftPageDown,
    CtrlShiftUp,
    CtrlShiftDown,
    Escape,
}

impl From<SpecialKey> for InputEvent {
    fn from(key: SpecialKey) -> Self {
        match key {
            SpecialKey::Backspace => InputEvent::Backspace,
            SpecialKey::Delete => InputEvent::Delete,
            SpecialKey::ArrowLeft => InputEvent::ArrowLeft,
            SpecialKey::ArrowRight => InputEvent::ArrowRight,
            SpecialKey::ArrowUp => InputEvent::ArrowUp,
            SpecialKey::ArrowDown => InputEvent::ArrowDown,
            SpecialKey::ShiftArrowLeft => InputEvent::ShiftArrowLeft,
            SpecialKey::ShiftArrowRight => InputEvent::ShiftArrowRight,
            SpecialKey::CmdArrowLeft => InputEvent::CmdArrowLeft,
            SpecialKey::CmdArrowRight => InputEvent::CmdArrowRight,
            SpecialKey::CmdShiftArrowLeft => InputEvent::CmdShiftArrowLeft,
            SpecialKey::CmdShiftArrowRight => InputEvent::CmdShiftArrowRight,
            SpecialKey::CmdArrowUp => InputEvent::CmdArrowUp,
            SpecialKey::CmdArrowDown => InputEvent::CmdArrowDown,
            SpecialKey::CmdShiftArrowUp => InputEvent::CmdShiftArrowUp,
            SpecialKey::CmdShiftArrowDown => InputEvent::CmdShiftArrowDown,
            SpecialKey::Home => InputEvent::Home,
            SpecialKey::End => InputEvent::End,
            SpecialKey::PageUp => InputEvent::PageUp,
            SpecialKey::PageDown => InputEvent::PageDown,
            SpecialKey::Enter => InputEvent::Enter,
            SpecialKey::Tab => InputEvent::Tab,
            SpecialKey::CmdA => InputEvent::CmdA,
            SpecialKey::CmdC => InputEvent::CmdC,
            SpecialKey::CmdX => InputEvent::CmdX,
            SpecialKey::CmdV => InputEvent::CmdV,
            SpecialKey::CmdShiftV => InputEvent::CmdShiftV,
            SpecialKey::CtrlC => InputEvent::CtrlC,
            SpecialKey::CtrlX => InputEvent::CtrlX,
            SpecialKey::CtrlV => InputEvent::CtrlV,
            SpecialKey::CtrlShiftV => InputEvent::CtrlShiftV,
            SpecialKey::OptionLeft => InputEvent::OptionLeft,
            SpecialKey::OptionRight => InputEvent::OptionRight,
            SpecialKey::ShiftHome => InputEvent::ShiftHome,
            SpecialKey::ShiftEnd => InputEvent::ShiftEnd,
            SpecialKey::CtrlShiftHome => InputEvent::CtrlShiftHome,
            SpecialKey::CtrlShiftEnd => InputEvent::CtrlShiftEnd,
            SpecialKey::CmdShiftHome => InputEvent::CmdShiftHome,
            SpecialKey::CmdShiftEnd => InputEvent::CmdShiftEnd,
            SpecialKey::FnUp => InputEvent::FnUp,
            SpecialKey::FnDown => InputEvent::FnDown,
            SpecialKey::ShiftPageUp => InputEvent::ShiftPageUp,
            SpecialKey::ShiftPageDown => InputEvent::ShiftPageDown,
            SpecialKey::CtrlShiftUp => InputEvent::CtrlShiftUp,
            SpecialKey::CtrlShiftDown => InputEvent::CtrlShiftDown,
            SpecialKey::Escape => InputEvent::Escape,
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_input_event_creation() {
        let char_event = InputEvent::from_char('a');
        assert_eq!(char_event, InputEvent::Character('a'));
    }

    #[test]
    fn test_special_key_conversion() {
        let backspace_event: InputEvent = SpecialKey::Backspace.into();
        assert_eq!(backspace_event, InputEvent::Backspace);
        
        let arrow_event: InputEvent = SpecialKey::ArrowLeft.into();
        assert_eq!(arrow_event, InputEvent::ArrowLeft);
    }

    #[test]
    fn test_cmd_a_select_all_event() {
        let select_all_event: InputEvent = SpecialKey::CmdA.into();
        assert_eq!(select_all_event, InputEvent::CmdA);
    }
}